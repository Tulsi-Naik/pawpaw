...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingPet) {
      res = await fetch(`http://localhost:5000/api/pets/${editingPet}`, {
        method: "PUT",
        headers: {
      ...
      res = await fetch("http://localhost:5000/api/pets/add", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
...
if (res.ok) {
  ...
  const data = await fetch("http://localhost:5000/api/pets/my", {
    headers: { Authorization: `Bearer ${token}` }
  }).then(r => r.json())
