...
const [rejectReason,setRejectReason] = useState("")
  const loadApps = async () => {
    try{
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/applications?status=${status}`)
      setApps(res.data)
    }catch{
  ...
  const loadPendingCount = async () => {
    try{
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/applications?status=pending`)
  ...
  const approve = async (id) => {
    try{
      await axios.put(`${import.meta.env.VITE_API_URL}/api/applications/${id}/approve`)
...
const reject = async () => {
  try{
    ...
    await axios.put(
      `${import.meta.env.VITE_API_URL}/api/applications/${rejectId}/reject`,
      { reason: rejectReason }
    )
