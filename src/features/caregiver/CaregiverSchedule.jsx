...
export default function CaregiverSchedule() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/bookings/my-assignments", {
...
.map(b => (
  ...
  await fetch(`http://localhost:5000/api/bookings/${b._id}/start`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${token}` }
  })
...
setBookings(prev =>
  prev.map(b =>
    ...
    {b.status === "InProgress" && (
      <button
        onClick={async () => {
          await fetch(`http://localhost:5000/api/bookings/${b._id}/complete`, {
            method: "PUT",
            headers: { Authorization: `Bearer ${token}` }
          })
